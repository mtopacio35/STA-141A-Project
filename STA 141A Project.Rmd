...
```{r, echo=FALSE}
setwd("C:/Users/19162/Downloads/sessions")

session = list()
for(i in 1:18){
  session[[i]] = readRDS(paste("C:/Users/19162/Downloads/sessions/session", i, ".rds", sep=""))
...
#Average Spike per Trial for Session 3
```{r, echo=FALSE}
avg_spks = list()
for(i in 1:18){
...
```{r, echo=FALSE}
#calculate difference in average spikes from trial to trial for the first session
...
```{r, echo=FALSE}
#create dataframe with avg spikes for all sessions
avg_spks_df = tibble(
  session = integer(), 
  avg_spks = numeric()
)
...
```{r, echo=FALSE}
#Initialize a list to store summary data for each trial
session_summary = list()

# Loop over each session
for(i in 1:18){
  #initialize an empty data frame to store summary data for each trial
...
for(j in 1:length(session[[i]]$feedback_type)){
  #compute summary statistics for the spks matrix for the current trial
...
#combine summary data for all sessions into a single data frame
session_all = bind_rows(session_summary)
session_all
```
The following table combines together into one, allowing for the display of session number, feedback type, the contrast levels, and the mean and standard deviations of the spikes. 
...
#Select variables for PCA and scale them
...
#view summary of the PCA result
summary(PCA.result)

#Create data frame for plotting
PCA.df = as.data.frame(PCA.result$x)

#Add session number to the data frame for coloring
PCA.df$session_number = session_all$session_number

#Create scatterplot of first 2 principal components
...
#Perform k-means clustering on the spike counts
...
#Add the cluster assignments to the data frame
    ...
    for(i in 1:n_neuron){
        ...
        ids.spk = which(spks[i,] > 0) # find out when there are spikes
...
#Convert FeedbackType to a numeric time series
...
#Assume you have a dataframe with your data
...
```{r, echo=FALSE}
# Perform generalized linear model (binomial family) for all sessions
glm.results = glm(binary_feedback ~ contrast_left + contrast_right + spks_mean + spks_sd, data = session_all, family = binomial)

# Print the summary of the generalized linear model
summary(glm.results)
```
...
```{r, echo=FALSE}
# Perform linear regression for all sessions
...
#Create empty list to store session-level data
session.data = list()

#Iterate over each session, prep data for modeling
for(i in 1:length(session)) {
  #extract relevant variables from session data
...
# Combine session-level data into single data frame
comb_data = do.call(rbind, session.data)

# convert feedback type to a factor w/ levels 0 and 1
...
# Build predictive model using log. regression
...
```{r, echo=FALSE}
test1 <- readRDS("C:/Users/19162/AppData/Local/Temp/Temp1_test.zip/test1.rds")
test2 <- readRDS("C:/Users/19162/AppData/Local/Temp/Temp1_test.zip/test2.rds")
summary(test1)
summary(test2)

```
```{r, echo=FALSE}
setwd("C:/Users/19162/Downloads/test")
